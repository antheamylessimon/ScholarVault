...
// Proponents should not access this view
if ($user['role'] === 'Proponent') {
    echo "<div class='glass' style='padding: 2rem; text-align: center;'>Access Denied.</div>";
    exit();
}
?>
<div class="view-header">
    <div>
        ...
        <p>Works currently assigned to your position: <strong><?php echo htmlspecialchars($user['position'] ?? 'N/A'); ?></strong></p>
    </div>
</div>
...
<div class="glass" style="margin-top: 1.5rem; border-radius: 16px; overflow: hidden;">
    ...
    <div id="ongoingList" class="works-list">
        <!-- Dynamic loading -->
    </div>
...
if (currentUserPosition === 'Extension Coordinator') {
    currentWorkTypeTab = 'Extension';
}
...
async function loadOngoingWorks() {
    try {
        const response = await fetch('api/works.php?type=ongoing');
        let works = await response.json();
        
        // General Evaluator filtering
        if (currentUserRole === 'Evaluator') {
            works = works.filter(w => {
                const isCurrentStep = w.current_step === currentUserPosition;
                const hasEvaluated = parseInt(w.has_evaluated || 0) > 0;
                return isCurrentStep || hasEvaluated;
            });
        }
        allOngoingWorks = works;
...
function populateYearDropdown() {
    ...
    const years = [...new Set(allOngoingWorks.map(w => w.year))].sort((a, b) => b - a);
...
function applyFiltersAndSort() {
    ...
    let filtered = allOngoingWorks.filter(work => {
...
function renderWorks(works) {
    ...
    if (works.length === 0) {
        list.innerHTML = `<div style="grid-column: 1/-1; text-align: center; padding: 3rem; opacity: 0.5;">No works found matching your criteria.</div>`;
        return;
    }
    ...
    list.innerHTML = works.map(work => {
        ...
        return `
            <div class="work-list-item" onclick="viewWorkDetails(${work.id})">
                <div class="work-list-icon"><i class="fas fa-file-signature"></i></div>
                ...
                <div class="work-list-status" style="text-align: right;">
...
// Override global reload for this page
window.loadWorks = loadOngoingWorks;
